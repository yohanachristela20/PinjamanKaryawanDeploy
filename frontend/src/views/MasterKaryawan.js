...
function MasterKaryawan() {
  ...
  const getKaryawan = async () =>{
    ...
    try {
      ...
      const response = await axios.get("https://04c4-103-141-189-170.ngrok-free.app/karyawan", {
        headers: {
  ...
  const deleteKaryawan = async(id_karyawan) =>{
    try {
      await axios.delete(`https://04c4-103-141-189-170.ngrok-free.app/karyawan/${id_karyawan}` , {
        headers: {
          Authorization: `Bearer ${token}`,
      },
      }); 
